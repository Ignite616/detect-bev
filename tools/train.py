...
def parse_args():
    ...
    # parser.add_argument(
    #     '--load-from', default='cascade_mask_rcnn_r18_fpn_coco-mstrain_3x_20e_nuim_bbox_mAP_0.5110_segm_mAP_0.4070.pth', help='the checkpoint file to load from')
    parser.add_argument(
...
def main():
    ...
    # set cudnn_benchmark;没有用到cuda加速？ 
    # if cfg.get('cudnn_benchmark', False):   # TODO:直接运行代码时，if条件不成立。跳过后torch.backends.cudnn.benchmark显示为false
    ...
    logger.info('Environment info:\n' + dash_line + env_info + '\n' +
                dash_line)  # 输出本地环境配置信息
    meta['env_info'] = env_info
    meta['config'] = cfg.pretty_text

    # log some basic info
    logger.info(f'Distributed training: {distributed}')
    logger.info(f'Config:\n{cfg.pretty_text}') # 输出网络结构信息
    ...
    # model.init_weights()  # 初始化的resnet50,那cascade_mask_rcnn_resnet18有什么用?
    logger.info(f'Model:\n{model}')
    ...
    # 原始加载的数据文件里，没有'prev'信息
    datasets = [build_dataset(cfg.data.train)]  #TODO：已经将训练集打乱，并且由323帧，变成了1690帧；在datasets_map2中初始化了BEV方格，类别与数字的对应关系
    if args.debug:
        dataset = datasets[0]
