...
""" 
运行的命令python tools/analysis_tools/fast-bev_vis.py test_result/m4_epoch_20_result/results_nusc.json --draw-gt
"""
import argparse
import json
import os
...
def parse_args():
    ...
    parser.add_argument(
        '--fps', type=int, default=3, help='Frame rate of video') # TODO：原先是3fps
    parser.add_argument(
...
def main():
    ...
    # info_path = \
    #     args.root_path + '/nuscenes_infos_%s_4d_interval3_max60.pkl' % args.version
    ...
    # 下面这三个参数用的parse中的默认参数
    scale_factor = args.scale_factor # 4
    canva_size = args.canva_size # 1000
    show_range = args.show_range # 50
    ...
    for cnt, infos in enumerate(
        ...
        # ] # 用来在fast-bev中盒子的计算是否合理

        # fast-bev测试结果中['translation'][2]表示的是物体高度的中心，不是物体底盘的中心。
        # 使用LiDARInstance3DBoxes类将标签，转为box时，用的是物体底盘中心，所以这里需要以下转换。
        for rid in range(len(pred_res)):
            pred_res[rid]['translation'][2] = pred_res[rid]['translation'][2] - pred_res[rid]['size'][2]/2
        pred_boxes = [
            pred_res[rid]['translation'] + pred_res[rid]['size'] + [
                Quaternion(pred_res[rid]['rotation']).yaw_pitch_roll[0]
            ] for rid in range(len(pred_res))
        ...
        if len(pred_boxes) == 0:   
            ...
            boxes = LB(pred_boxes, origin=(0.5, 0.5, 0.0)) # 用iDARInstance3DBoxes处理以下数据
            corners_global = boxes.corners.numpy().reshape(-1, 3) # （78，8，3）->(624,3)
