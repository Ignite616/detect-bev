    ...
    def grid_anchors(self, featmap_sizes, device='cuda'):
        ...
        for i in range(self.num_levels):
            anchors = self.single_level_grid_anchors(
                featmap_sizes[i], self.scales[i], device=device) # ([1, 100, 100, 4, 2, 9])
            if self.reshape_out:
        ...
        for anchor_range, anchor_size in zip(self.ranges, self.sizes):
            mr_anchors.append(
                self.anchors_single_range(
                    ...
                    anchor_size,
                    self.rotations,
                    device=device))  # 每一个anchor的shape 4 x ([1,100,100,1,2,9])
        mr_anchors = torch.cat(mr_anchors, dim=-3) # ([1,100,100,4,2,9])
        return mr_anchors
