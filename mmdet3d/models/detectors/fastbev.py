...
class FastBEV(BaseDetector):
    ...
    def extract_feat(self, img, img_metas, mode):
        ...
        for lvl, mlvl_feat in enumerate(mlvl_feats):  
            ...
            mlvl_feat = mlvl_feat.reshape([batch_size, -1] + list(mlvl_feat.shape[1:])) # ([24, 64, 80, 220]) -> ([1，24, 64, 80, 220])
            # [bs, seq*nv, c, h, w] -> list([bs, nv, c, h, w])
            mlvl_feat_split = torch.split(mlvl_feat, 6, dim=1) # ([1，24, 64, 80, 220]) ->  ([1，6, 64, 80, 220])
            ...
            for seq_id in range(len(mlvl_feat_split)):
                ...
                for batch_id, seq_img_meta in enumerate(img_metas):
                    ...
                    if self.style in ['v1', 'v2']:
                        # wo/ bev ms
                        n_voxels, voxel_size = self.n_voxels[0], self.voxel_size[0] # (200,200,4). (0.5,0.5,1,5)
                    else:
                        # v3/v4 bev ms
    ...
    def forward_train(
        ...
        feature_bev, valids, features_2d = self.extract_feat(img, img_metas, "train") # 使用m1的config时，feature_bev：[(1,192,100,100)] ;valids, features_2d均为none
        """
        ...
        if self.bbox_head is not None:
            x = self.bbox_head(feature_bev) # m1模型下，x是一个列表，长度为3。cls_scores: ([1,80,100,100],bbox_preds:[1,72,100,100],dir_cls_preds:[1,16,100,100])
    ...
    def simple_test(self, img, img_metas):
        bbox_results = []
        feature_bev, _, features_2d = self.extract_feat(img, img_metas, "test")  # feature_bev(1,c,100,100)
        if self.bbox_head is not None:
            x = self.bbox_head(feature_bev) # 通过piontpillar检测头的卷积核预测三个分数，cls_score # (1,80,100,100)， bbox_pred # (1,72,100,100) dir_cls_preds # (1,16,100,100)
            bbox_list = self.bbox_head.get_bboxes(*x, img_metas, valid=None) # 根据上面的三个预测值，和anchor，得到最终的box
            bbox_results = [
...
def get_points(n_voxels, voxel_size, origin):
    ...
    points = points * voxel_size.view(3, 1, 1, 1) + new_origin.view(3, 1, 1, 1) # （3，200，200，4）
    return points
...
def backproject_inplace(features, points, projection):
    ...
    function: 2d feature + predefined point cloud -> 3d volume
    input:
        features: [n_images, n_channels, height, width]
        points: [3, 200, 200, 12]
        projection: [6, 3, 4]
    output:
    ...
    ).type_as(features)  # [n_channels, 480000]
    for i in range(n_images):
