...
class FreeAnchor3DHead(Anchor3DHead):
    ...
    def loss(self,
        ...
        anchors = [torch.cat(anchor) for anchor in anchor_list] # ([80000, 9])

        # concatenate each level, ([1, 80, 100, 100]) -> ([1, 80000, 10])
        cls_scores = [
        ...
        for _, (anchors_, gt_labels_, gt_bboxes_, cls_prob_, bbox_preds_,
            ...
            loss_bbox = self.loss_bbox(
                ...
                matched_object_targets,
                bbox_weights,
                reduction_override='none').sum(-1)  # (N,25)，N是真值框的个数，25是self.pre_anchor_topk
